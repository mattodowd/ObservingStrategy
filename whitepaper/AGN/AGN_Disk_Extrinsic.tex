...
%Suggestions for further motivation:
%Measurement of accretion disk temperature gradient and size will provide a powerful constraint on the physics of accretion onto the black hole, including the efficiency of black hole feeding and hence growth. In addition, high magnification events will resolve inhomogeneities in accretion disks such as hot-spots and warped geometries, as well as discontinuities such as the ISCO shadow and evacuated regions due to a secondary SMBH or IMBH.
%If you think this last point is worth expanding on we should talk about the potential resolution of these inhomogeneities that could be detected with LSST.
...
% [Matt] An advantage of the continuous monitoring is that it may be possible to calibrate out the intrinsic variability,
% assuming time delays are known or can be measured. 
